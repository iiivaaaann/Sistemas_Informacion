...
def ejercicio4(): #Falta el ultimo apartado.
    ...
    #df.index=df['time']
    #print(df)
    #df=pd.read_sql_query("SELECT TIME as time, COUNT(*) as N_ALERTS FROM ALERTS GROUP BY STRFTIME('%YYYY-%MM-DD HH', time) order by 1;", conexion)
    #print(df["time"])
    #plt.figure(num=None, figsize=(14, 6), dpi=80, facecolor='w', edgecolor='k')
    #plt.plot(df["time"], df["N_ALERTS"], linewidth=3)
    #print(df)
    #plt.show()
    df=pd.read_sql_query("SELECT * FROM ALERTS", conexion)
    df['TIME'] = pd.to_datetime(df['TIME'])
    df_agrupado_2dias = df.groupby(pd.Grouper(key='TIME', freq='2D')).size().reset_index(name='COUNT')
    print(df_agrupado_2dias)
    plt.figure(num=None, figsize=(20, 10), dpi=80, facecolor='w', edgecolor='k')
    plt.plot(df_agrupado_2dias["TIME"], df_agrupado_2dias["COUNT"], linewidth=3)
    plt.show()
    exit(0)
